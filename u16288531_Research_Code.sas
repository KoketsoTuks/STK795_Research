...
*Template for the stepwise confusion matrix;
PROC TEMPLATE;
...
*Prepare data;
DATA HEART(where=(AgeCHDdiag is not null)) ;
SET SASHELP.HEART;
...
TABLE Status Sex / NOCUM;
RUN;

*descriptive statistics;
Title '';
PROC MEANS DATA=HEART MAXDEC=2 N Mean Median stddev Min Max;
VAR AgeCHDdiag Cholesterol Diastolic Systolic Smoking BMI;
RUN;

*correlations between variables;
PROC CORR DATA=HEART NOSIMPLE;
VAR AgeCHDdiag Cholesterol Diastolic Systolic Smoking BMI ;
RUN;

*multicollinearity diagnostics;
PROC REG DATA=HEART Plots=None;
MODEL BMI = AgeCHDdiag Cholesterol Diastolic Systolic Smoking / VIF TOL;
RUN;
...
PROC LOGISTIC DATA=HEART;
	CLASS Sex
		/ PARAM=ref;
	MODEL Status(EVENT='Alive')=Sex AgeCHDdiag Diastolic Systolic BMI 
		Smoking Cholesterol / LINK=logit TECHNIQUE=fisher;
RUN;
...
PROC LOGISTIC DATA=HEART;
	CLASS Sex
		/ PARAM=ref;
	MODEL Status(EVENT='Alive')=Sex AgeCHDdiag Diastolic Systolic BMI 
		Smoking Cholesterol / SELECTION=stepwise LINK=logit  OUTROC=test_roc TECHNIQUE=fisher;
		OUTPUT OUT=stepwisepredicted P=P PREDPROBS=I;
RUN;

*confusion matrix;
PROC FREQ DATA=stepwisepredicted;
TABLES _INTO_*_FROM_ / NOPERCENT NOCUM NOCOL NOROW ;
RUN;
...
*call the macro;
%GainLift(data=stepwisepredicted, response=status, p=p, event="Alive")
